...
\textit{close} transition. That's the reason why the simulation wasn't able to perform the
...
To conclude, the simulation wasn't able to test the transition \textit{close} since the
...
for \textit{interest} transition isn't successful and the HTTP status code returned 400
...
Although, the request made for the transition \textit{interest} isn't
...
which code isn't correctly generated. The generated code for the \textit{interest} event
...
isn't accepted by the \gls{sut}. For example, a generated pre- or postconditions
aren't satisfied by the execution from the traces.

In this experiment we've found bugs in the code generators. For both bugs,
...
\gls{smt} solver. The preconditions aren't satisfied for both bugs. To conclude,
we did find bugs as expected where the \gls{sut} wasn't conform to the
specification.

This experiment is using more complex specifications which implements
...
tested on all generators. As expected, the synchronisation is tested in the
\gls{sut}, but no faults were found. There does not seem to be any faults in
partial failure or asynchrony. In our case, the expectation is to find bugs in
synchronisation where the \gls{sut} doesn't produce a unanimous final outcome.
In the case of the expectation of the \gls{sut} producing not unanimous final
outcome, no faults were found here either.
...
\gls{sut} runs. The \gls{sut} is run as single nodes, both the Scala system and
Cassandra. By running multiple nodes, it is possible to focus more on the
testing of unanimous final outcome of the \gls{sut}s. Unfortunately, this is
currently not possible because the \gls{sut} cannot run distributed and the test
...
execution from the trace. In this experiment are the transitions in the
...
parameters data values generated by the \gls{smt} solver. In this experiment are
...
Also with this experiment we've found bugs in the code generators. The bug
...
experiment, we've found bugs for both categories.

As discussed before, with this experiment it wasn't possible to test the
