...
of concept, we are going to use more complex specifications, complex in the sense
...
By using the traces, it also solves the problems from the previous proof of
...
\subsection{Distributed Codegen-Akka}
...
With this test, we will run the \gls{sut} distributed generated by the
...
synchronisation where the \gls{sut} does not produce a unanimous final outcome.
...
\gls{sut}. The \gls{smt} solver can be configured to use random seeds
