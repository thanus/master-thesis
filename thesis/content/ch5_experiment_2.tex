\chapter{Experiment 2: Valid execution}\label{sec:ch5}
...
final state failed. To successfully execute a transaction is the transition
...
Another difference in the transaction specification is that transition definitions
can contain sync expressions. From the previous transition definitions, we have only
...
These formulas do not have logic about the implementation of synchronisation.
Of course, the \gls{sut} has implemented synchronisation for these transitions.
So it is possible to also test synchronisation in the \gls{sut}.
There are also several studies which report
...
This allows the \gls{sut} to run distributed to test with our approach.
...
\end{lstlisting}
\caption{\textit{book} transition definition from transaction specification}\label{fig:transaction-book-event}
\end{sourcecode}
\FloatBarrier
...
approach, the traces from the \gls{smt} solver are used to check whether the
\gls{sut} accepts the execution from the traces, and whether the execution is
conforms to the specification.~\cite[p.~5]{stoel_storm_vinju_bosman_2016}
...
% Hier komen implementatie concepten bij kijken die niet triviaal zijn zoals “transactions” en “locking” (citeer een boek of zo). Verder zou je termen zoals “race conditie” en “deadlock” kunnen gebruiken/uitleggen (misschien in het volgende hoofdstuk). Een deadlock leidt er toe dat de simulatie wel een stap kan zetten maar de implementatie niet meer. een race kan je zien als de implementatie andere waarden gaat produceren dan de simulatie.
...
transition needs to be tested. Some transitions require an initial state for
which transitions need to be executed to reach this state.
As a result, previously executed transitions can be executed again.
...
the simulation and checking are requested for a test case for a chosen transition.
The simulation and checking provide then a trace (test case) for the requested transition.
...
\begin{figure}[h!]
  ...
  \caption{Testing approach valid execution}\label{fig:simulation-testing}
\end{figure}
\FloatBarrier

As discussed before, the study~\cite[p.~5]{stoel_storm_vinju_bosman_2016}
carried out a model testing approach to test existing banking systems.
Although, in this approach, it was only possible to test the \gls{sut}
interactively using the simulation.
The traces from the \gls{smt} solver are used in this approach to check whether the
\gls{sut} accepts the execution from the traces, and whether the execution is
conforms to the specification.~\cite[p.~5]{stoel_storm_vinju_bosman_2016}
Thus the traces are played in the \gls{sut}.

In our approach to playback the steps from a given trace on the \gls{sut}, we
are going to split every transition into three steps as in the
...
parameters to reach the resulting state, $s_{2}$.
...
checker. The model checker provides traces when the current state is
satisfiable.
Then the traces are executed on the \gls{sut}, which is in more depth described
in the section \autoref{sec:ch5-traces}. Performing each step from a trace
entails the three steps to test and execute the transition.
...
\end{lstlisting}
\caption{A tebl for the current state for the transition \textit{book} generated
by the test case generator}\label{fig:tebl-gen-validated-transaction}
\end{sourcecode}
\FloatBarrier
...
In cooperation with the author of the study~\cite{stoel_storm_vinju_bosman_2016}
...
the transition: $p(s_{1}, s_{2})$
\item Check whether the invariants hold in the state after the transition:
$P(s_{2})$. The function $P$ is the invariants (safety properties).
\end{enumerate}
...
and the trace are compared to find any misbehaviour in the \gls{sut}.
...
specification. To each state and transition is a distinct identity assigned. The
...
state.
To compare the current state from the \textit{\_state} field, the distinct
identity of \textit{\_state} field needs to be
converted to compare it to the \gls{sut}.

% Adding Frame Conditions. To guard the fields that are not changed by the transition frame conditions are added (Jack- son 2012). These frame conditions make sure that a field has the same value after the transition as before.
...
Codegen-Akka generator. In this test run is the \gls{sut} run in a distributed
mode as multiple nodes, both the Scala system and Cassandra.
...
\end{lstlisting}
\caption{Test run for the \textit{close} transition (part 1)}\label{fig:result-codegenakka-close-p1}
\end{sourcecode}
\FloatBarrier
...
1:
  ...
  instance: simple_transaction.Account, key = FO9402337176862639
    ...
    var balance (type: Money) = EUR50.00
\end{lstlisting}
\caption{Test run for the \textit{close} transition (part 2)}\label{fig:result-codegenakka-close-p2}
\end{sourcecode}
\FloatBarrier
...
specified with the identifier of the account. As we have seen the transition definition
...
check doCheck reachable in max 4 steps;
\end{lstlisting}
\caption{Generated tebl for the transition \textit{book}}\label{fig:tebl-gen-validated-transaction}
\end{sourcecode}
\FloatBarrier

\subsubsection{Inconclusive state}
...
\end{lstlisting}
\caption{Test run with inconclusive states}\label{fig:result-not-found-state}
\end{sourcecode}
\FloatBarrier
...
\end{lstlisting}
\caption{Test run for the \textit{interest} transition with the use of javadatomic generator (part 1)}\label{fig:result-javadatomic-interest-p1}
\end{sourcecode}
\FloatBarrier
...
\end{lstlisting}
\caption{Test run for the \textit{interest} transition with the use of javadatomic generator (part 2))}\label{fig:result-javadatomic-interest-p2}
\end{sourcecode}
\FloatBarrier
...
The transition definition for the \textit{interest} transition is given in
\autoref{fig:account-interest-event}. This transition definition states that the
...
\end{lstlisting}
\caption{\textit{Interest} transition definition from account specification}\label{fig:account-interest-event}
\end{sourcecode}
\FloatBarrier

Now we know that we have discovered a fault since the simulated transition is
conform to the specification, we want to know where this misbehaviour occurs and
which code is not correctly generated. The generated code for the \textit{interest} transition
...
\end{lstlisting}
\caption{Generated precondition of the \textit{interest} transition}\label{fig:java-gen-interest-pre}
\end{sourcecode}
\FloatBarrier
...
\end{lstlisting}
\caption{Method used by the generated precondition from \autoref{fig:java-gen-interest-pre}}\label{fig:java-less-or-equal-check}
\end{sourcecode}
\FloatBarrier
...
"message":"OK","errorBody":"","code":"200")
\end{lstlisting}
\caption{Test run for the \textit{interest} transition with the use of Scala-ES generator (part 1)}\label{fig:result-scalaes-interest-p1}
\end{sourcecode}
\FloatBarrier
...
\end{lstlisting}
\caption{Test run for the \textit{interest} transition with the use of Scala-ES generator (part 2)}\label{fig:result-scalaes-interest-p2}
\end{sourcecode}
\FloatBarrier
...
\end{lstlisting}
\caption{Generated Interst class}\label{fig:scala-gen-interest-json}
\end{sourcecode}
\FloatBarrier
...
\end{lstlisting}
\caption{Request handler for the transition \textit{interest}}\label{fig:scala-gen-interest-request}
\end{sourcecode}
\FloatBarrier
...
\end{lstlisting}
\caption{Test run for the transition \textit{interest} with the use of distributed Codegen-Akka}\label{fig:result-dist-codegenakka-interest}
\end{sourcecode}
\FloatBarrier
...
[info] akka.remote.MessageSerializer$SerializationException: Failed to serialize remote message
  ...
  [class com.romix.akka.serialization.kryo.KryoSerializer].
\end{lstlisting}
\caption{Serialization error in Node 1}\label{fig:dist-interest-serialization}
\end{sourcecode}
\FloatBarrier

When the post-transition check is performed on Node 2, an exception is thrown
which is shown in \autoref{fig:dist-interest-serialization}. This exception is a
serialisation error which is thrown because it is not able to serialise the
\textit{Rebel} instance account. It is remarkable that this exception appears in
Node 1 when the post-transition check is performed on Node 2. Thus it is not
possible to retrieve an instance from Node 2 because of a serialisation error.

Now we have only discussed the test results of the \textit{interest} transition.
According to the results of \autoref{sec:ch5-run-dist-codegenakka}, the other
transitions also fail. The reason for the failing tests for the transitions is
the same as for transition \textit{interest}.
Except for the transition \textit{close}, as mentioned earlier, the simulation
is not able to simulate the transition. To conclude, in this experiment the
\gls{sut}'s does not produce a unanimous final outcome.
...
The cause of this is serialisation errors in the nodes.
...
Also with this experiment, we have found faults in the code generators. The fault
