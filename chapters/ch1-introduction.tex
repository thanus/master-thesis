...
Growing systems is a concern for large organizations \cite[p.1]{stoel_storm_vinju_bosman_2016}. The continuity of systems becomes difficult and a single modification can result in unexpected behaviour of a larger part of the system. Within the domain knowledge, reasoning about the expected behaviour of a system, changes and errors are hard. Rebel aims to solve these challenges by centralizing the domain knowledge and relating it to the running systems. Rebel is a formal specification language which is used for controlling the intrinsic complexity of software for financial enterprise systems \cite[p.1]{stoel_storm_vinju_bosman_2016}.
Software testing is an important part in software projects \cite[p.4]{myers2011art}. The testing process within large systems can be challenging, it entails not only defining and executing a lot of test cases, solving thousands of errors, handling thousands of modules, but also enormous project management. To facilitate this process Rebel offers automated simulation and checking of specifications with the use of a Satisfiability Modulo Theories (SMT) solver. This solves to some extent the testing and reasoning of Rebel specifications, but this is only within in the Rebel domain. With the use of code generators, code can be generated from specifications. The problem with code generation is the resulting product is leaving the Rebel domain, causing loss of testing and reasoning with the use of formal methods. 
The challenge is to regain the benefits from the Rebel domain to be able to test and reason about running systems.
...
% The research is about proving the implementation correctness of a specification. \textbf{Research question:} How do you efficiently find out whether the generated code conforms to the specification?
...
As mentioned before, the simulation and checking for the correctness of specifications is only in the Rebel domain. The running systems which are generated from specifications need to be properly based on these specifications, it should conform to these specifications. So additional work is necessary for the generation process to know that running systems are conforming to the specifications. The language Rebel promised to be deterministic, this also holds for the generated system. So non-deterministic behaviour should be identified. For ING Bank it is especially important that there is no corrupted data within the runtime systems \unsure{is dit nog nodig om te vermelden?}.

% In \cite[p.3]{stoelcase}, a possible solution for these problems might be to use SMT Solvers in this process. 

% The research is about testing the implementation correctness of a specification, by comparing the generated system against the SMT Solver. As before mentioned, the mapping of the Rebel language to the SMT formulas make it possible to check the specifications (bounded model checking). The formal verification is done by the model checking \cite{sanghavi2010formal}. In Rebel is bounded model checking used, since it checks only for invariant which doesn't hold. There exist different algorithms and approaches for software model checking \cite{beyer2016smt}. The SMT Constraints are generated from the specification and then solved by a SMT Solver, in our case the Z3 Theorem Prover \cite{moura_bjorner_2008}. Since the SMT Solver is already used to check the specification \cite{stoel_storm_vinju_bosman_2016}, it is a good solution to test the generated system against the SMT Solver. In order to do this, research will be carried to determine the input/output to generated system and SMT Solver, mapping (translation) of the input/output (traces), and to what extent does the comparison of these values prove the implementation correctness. Then the initial version can be improved by using software testing techniques like: \cite{sen2007concolic, jia2011analysis, godefroid2008automated}. With the approach there are a few assumptions made: 
% \begin{itemize}
% \item The specification is correct. An incorrect specification will probably not pass the bounded model checking, and testing a generated system derived from an incorrect specification isn't really effective.
% \item The system can be compiled. When the generated system cannot be compiled, then there is a fault in the code generator. Then testing this system is impossible since it cannot be compiled.
% \item The specification is correctly interpreted by the SMT Solver.
% \end{itemize}
...
%   how is input/output determined/compared? mapping?
  
  \item Sub. Are there any false positives/negatives when the generated system has been implemented correctly?
% generated system is goed. SMT beweert tegenovergestelde -> vraag
% false negatives false positives, smt move to research method

  \item Sub. What kind of bugs can be found and what are the factors?
%   What kind of bugs do you expect to find?  

% validation/evaluation
%   \item Sub. How many mistakes can you find and what are the factors?
%   \item Sub. How fast do you find mistakes?  
\end{itemize}
