...
we know that the SUT contains the bug to close an account with some balance. For
the lightweight version it is expected to find this bug in the SUT.
...
isn't reachable. Although, the state in the SUT is reachable.

Looking at the account in the SUT, it looks as follows
...
Checking is used in this lighweight version to check whether it is possible
