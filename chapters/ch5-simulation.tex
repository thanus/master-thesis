...
test synchronisation in the SUT. There are also several studies which reports
...
SMT solver has performed. Then these transitions can be performed in the SUT.
...
SUT. Therefore, the expectation is to perform the same transition from the traces
...
which is then the SUT.

When the SUT is generated from the CSTs, the SUT can be tested by the test
...
To test the SUT using the generated test cases, the data
from these test cases are provided to the adapter. Finally, the adapter
communicates this data to the SUT.
...
case in the SUT since a current state needs to be initialised first before the
transition is performed.

To initialise the current state in the SUT checking can be used. By defining
...
it is necessary to check the new state in the SUT. Therefore, an adapter
