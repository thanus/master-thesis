\chapter{Experiment 2: Valid execution traces}
\label{sec:ch5}
...
SMT solver has performed. Then these transitions can be performed in the \gls{sut}.
...
\subsubsection{Bugs}
In this approach we're using the traces from the SMT solver to test the \gls{sut},
thus testing what should be possible according to the specification. The
expectation here is to find bugs in the \gls{sut} which doesn't accept the execution
from the traces.

\subsubsection{Efficiency}
By using the traces, we check whether the given execution is possible in the
\gls{sut}. Therefore, the expectation is to perform the same transition from the traces
should be performed in the \gls{sut}.
...
are being built, \textit{i.e.}, Concrete Syntax Trees (CSTs) are produced of
these specifications. Using these CSTs, the code generator generates the code,
which is then the \gls{sut}.

When the \gls{sut} is generated from the CSTs, the \gls{sut} can be tested by the test
framework. The test case generator traverses the CSTs of the specifications to
generates test cases for all transitions. Using the information from the CSTs,
...
from these test cases are provided to the adapter. Finally, the adapter
communicates this data to the \gls{sut}.
...
\autoref{sec:ch5-traces}. Performing each step from a trace entails the three steps to test and
...
the \gls{sut}. Therefore, \textit{JSON} should be generated for the chosen transition.
...
normalised. This normalisation process is done to make the SMT formulas easier
...
\subsubsection{Test case adapter}
As discussed in the code generation section, the request made to the API of the
\gls{sut} are "standardized", but the response isn't. For the post-transition check,
it is necessary to check the new state in the \gls{sut}. Therefore, an adapter
needs to be defined to be able to compare the results of the \gls{sut} and the trace.
...
\end{lstlisting}
\caption{Account state in the \gls{sut} after performing the interest transition}
...
find bugs in the \gls{sut} where execution traces from the SMT solver isn't accepted
by the \gls{sut}.
...
SMT solver. To conclude, we did find bugs as expected where the \gls{sut} wasn't
conform to the specification.

\subsection{Efficiency}
With the use of traces it is expected to perform the same transition from the
traces in the \gls{sut}.
...
\textit{close} transition. The precondition of this transition isn't
